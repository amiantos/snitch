...
class DiscordBridge {
  constructor(logger, config, topicStore = null) {
    ...
    this.adminPersistentTopic =
      this.adminEnabled && this.topicStore
        ? this.topicStore.get(this.adminChannel)
        : null;
  ...
  _handleTopicCommand(event, args) {
    ...
    if (!this.topicStore) {
      this.logger.error("EyeBridge !topic: no topic store available for persistence");
      this._notice(event.nick, "Topic persistence is unavailable.");
      return;
    }

    try {
      this.topicStore.set(this.adminChannel, args);
    } catch (err) {
      this.logger.error(
